...
class QubridConfig(BaseModel):
    ...
    api_key: str = Field(..., description="Qubrid API key")
    base_url: str = Field(default="https://api.qubrid.ai/v1")
    model: str = Field(default="Qwen3-Coder-30B-A3B")
...
def load_config() -> AppConfig:
    ...
    # Qubrid configuration (required)
    qubrid_config = QubridConfig(
        api_key=os.getenv("QUBRID_API_KEY", ""),
        base_url=os.getenv("QUBRID_BASE_URL", "https://platform.qubrid.com/api/v1/qubridai"),
        model=os.getenv("QWEN_MODEL", "Qwen/Qwen3-Coder-30B-A3B-Instruct"),
...
if __name__ == "__main__":
    ...
    print(f"  Qubrid Model: {config.qubrid.model}")
    print(f"  Output Directory: {config.project.output_dir}")
    print(f"  GitHub Enabled: {config.github.enabled}")
